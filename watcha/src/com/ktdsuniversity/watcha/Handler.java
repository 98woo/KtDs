...
public class Handler {
	...
	private void movieMenuHandler() {
		...
		if (movieMenu == 1) {
			...
			List<MoviesVO> movies = this.moviesService.findMoivesByTitle(movieTitle);
			...
			// TODO 영화 수정 로직 작성 해야함.... tdd작업방식 (역순으로 작업하는 방식)
			System.out.println("제목 (" + moviesVO.getTitle() + "): ");
			...
			if (newRunningTime != null && newRunningTime.trim().length() > 0) {
				moviesVO.setRunningTime(movieMenu);
			}
			...
			boolean updatedResult = this.moviesService.modifyOneMoive(moviesVO);
			if (updatedResult) {
			...
			boolean deletedResult = this.moviesService.deletedOneMovieById(movieId);
			if (deletedResult) {
			...
			boolean isCreate = this.moviesService.createNewMovie(newTitle, newMinimumAge, newOpenYear, newRunningTime, newGenre, newAtmosphere, newLocation, newSummary, newPoster, null);
	...
	private void directorMenuHandler() {
		...
		if (directorMenu == 1) {
			...
			List<DirectorsVO> directors = this.directorsService.findDirectorsAllMovies();
			if (directors.size() == 0) {
			...
			System.out.println("존재하는 않는 번호입니다.");
		}
	}
...
//		boolean ratingIsCreate = ratingsService.createNewRating("MV-20240229-000035", 4.5, "볼만해요.", "testuser");
//			if (ratingIsCreate) {
